...
static ztimer_now_t wrapper_thread2(void)
{
	...
	while (thread_getstatus(thread_pid) != (STATUS_STOPPED | STATUS_NOT_FOUND)) { }
	volatile ztimer_now_t after = ztimer_now(ZTIMER_USEC);
...
static void *work_csp(void *args, channel *c)
{
	(void)args;
	msg_t m = {0};
...
static void print_result(const char label[static const restrict 1], const size_t timestamp);
...
int main(void) {
	...
	size_t thread1_sum = 0;
	size_t thread2_sum = 0;
	size_t csp_sum = 0;
