 ...
 * 
 * Updated with CSP 2024 - Jonathan L. Claudius
 */
...
void * thread_worker(void * d, channel *c)
{
	(void)d;
...
{
	{
		...
		// thread_create(stack, sizeof(stack), 7, THREAD_CREATE_STACKTEST, thread_worker, &wc, "T1");

		static channel c1 = {0};
		c1 = channel_make(&c1, 1);
		channel_send(&c1, &wc, sizeof (wc));
		GO(thread_worker, nullptr, &c1);
	}
	{
		static struct worker_config wc = THREAD_2;   /* 0-10 workness */
		static channel c2 = {0};
		c2 = channel_make(&c2, 1);
		channel_send(&c2, &wc, sizeof (wc));
		GO(thread_worker, nullptr, &c2);
	}
	{
		static struct worker_config wc = THREAD_3;   /* 0-10 workness */
		static channel c3 = {0};
		c3 = channel_make(&c3, 1);
		channel_send(&c3, &wc, sizeof (wc));
		GO(thread_worker, nullptr, &c3);
	}
}
